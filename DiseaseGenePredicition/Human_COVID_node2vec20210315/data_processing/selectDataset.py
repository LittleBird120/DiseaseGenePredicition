#与partitionDataset不同的是那个文件里面是有数据分割获取对照组数据
import json
import math
import os
...
#对训练集进行网络构建
def getTrainNetwork(nodesDict,graph):
    trainfile = "../data/crossValidation/firstExperiments/train.txt"
    testfile = "../data/crossValidation/firstExperiments/test.txt"
    train_dataset = np.loadtxt(trainfile, dtype=np.str)
    test_dataset = np.loadtxt(testfile, dtype=np.str)
    ...
    #     if test_dataset[i] in trainHost:
    #         num.append(test_dataset[i])

    for i in range(len(trainHost)):
        for j in range(i+1, len(trainHost)):
            if graph.has_edge(trainHost[i], trainHost[j]):
    ...
    result_host_path = parent_path + r'\data\crossValidation\firstExperiments\train_host.json'
    with open(result_host_path, 'w') as fw:
        json.dump(trainHostDict, fw)

    result_virus_host_path = parent_path + r'\data\crossValidation\firstExperiments\train_virus_host.json'
...
def save_goSim(dict, pro):
    ...
    file_path = "../data/crossValidation/firstExperiments/train_COVID_AllHuman_GoSim.xls"
...
if __name__ == '__main__':
    ...
    trainHostDict, trainEdgesList = getTrainNetwork(nodesDict, G)
